						...
						>
							<MenuItem eventKey={subNav.key}> 
								{subNav.path 
									...
									?
										<Dropdown placement="middle-start-top" title={subNav.title}>
											{
												subNav.subMenu.map(item => (
													item.path 
													?
													<Dropdown.Item eventKey={item.key}>
														<Link 
															className="h-full w-full flex items-center" 
															onClick={() => onLinkClick?.(
																{
														...
														>
															<span>
																<Trans i18nKey={item.translateKey} defaults={item.title} />
															</span>
														</Link>
													</Dropdown.Item>
													:
